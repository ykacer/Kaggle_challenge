...
if descriptor_type == 'sift':
	...
	descriptor_extractor = cv2.SURF(1000)
...
bow_labels = desriptor_type+'_bow_labels.csv'
...
class_weight = dict(zip(classes,weights[1:]))
...
print np.array_str(metrics.confusion_matrix(ytest,cl.predict(Xtest)))

cl = KNeighborsClassifier()
cl.fit(Xtrain,ytrain)
print np.array_str(metrics.confusion_matrix(ytest,cl.predict(Xtest)))

cl = LogisticRegression(C=75.0,class_weight=class_weight,random_state=0)
cl.fit(Xtrain,ytrain)
print np.array_str(metrics.confusion_matrix(ytest,cl.predict(Xtest)))
